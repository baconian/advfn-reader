...
def parse_site(url):
    ...
    if soup.find_all(lambda tag: tag.name=='table' and tag.has_attr('cellpadding') and tag['cellpadding']=="2") != []:
        table = soup.find_all(lambda tag: tag.name=='table' and tag.has_attr('cellpadding') and tag['cellpadding']=="2")[1]
...
def get_links(ticker,exchange):
    ...
    url = "https://de.advfn.com/borse/" + exchange + "/" + ticker + "/finanzwerte?btn=istart_date&istart_date=1&mode=quarterly_reports"
    df = parse_site(url)
    start= dt.datetime.strptime(df.index[0], '%Y/%m')
    end = start + relativedelta(months=3)
    curr_date = dt.datetime.today().date()
    num_of_pages = (relativedelta(get_closest_quarter(curr_date), start.date()).years*4 + relativedelta(get_closest_quarter(curr_date), start.date()).months)
    links = []
    i=1
    while i< (num_of_pages):
        links.append("https://de.advfn.com/borse/" + exchange + "/" + ticker + "/finanzwerte?btn=istart_date&istart_date=" + str(i) +"&mode=quarterly_reports")
        i+=5
    return links


def get_quarts(ticker, exchange = 'NYSE'):
    """Perfoms the multiprocess parsing of a given ticker and exchange. Returns Pandas DataFrame."""
    quart_urls = get_links(ticker, exchange)
    with Pool(10) as p:
